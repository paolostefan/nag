...
MandelbrotEffect::MandelbrotEffect()
{
  ...
  parameters.emplace_back("zoom", ParameterType::FLOAT, 1.0f, 0.1f, 1000.0f, 0.1f);
  parameters.emplace_back("center", ParameterType::VEC2, Vec2{-0.5f, 0.0f}, Vec2{-2.0f, -2.0f}, Vec2{2.0f, 2.0f}, Vec2{.0001f, .0001f}, "%.6f");
  parameters.emplace_back("iterations", ParameterType::INT, 100, 10, 1000, 1);
  parameters.emplace_back("color_offset", ParameterType::FLOAT, 0.0f, -1.0f, 1.0f, 0.01f);
}
...
void MandelbrotEffect::render_quad()
{
  glBindVertexArray(vao);
...
void MandelbrotEffect::render(int width, int height, double time_ms)
{
  ...
  auto *zoom_param = get_parameter("zoom");
  if (zoom_param)
  {
    float zoom = std::get<float>(zoom_param->get_value());
    shader->set_uniform("u_zoom", zoom);
  }

  auto *center_param = get_parameter("center");
  if (center_param)
  {
    Vec2 center = std::get<Vec2>(center_param->get_value());
    shader->set_uniform("u_center", center.x, center.y);
  }

  auto *iter_param = get_parameter("iterations");
  if (iter_param)
  {
    int iterations = std::get<int>(iter_param->get_value());
    shader->set_uniform("u_iterations", iterations);
  }

  auto *color_param = get_parameter("color_offset");
  if (color_param)
  {
    float color_offset = std::get<float>(color_param->get_value());
