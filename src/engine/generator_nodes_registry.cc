...
namespace node_registration {
  void register_generator_nodes() {
    ...
    registry.register_node(NodeType::StepSequencer, "Generators", "Outputs a sequence of values based on a trigger input",
    ...
    registry.register_node(NodeType::ParticleEmitter, "Generators", "Emits 2D particles at a given rate with configurable speed and lifetime",
      [] { return ParticleEmitterNode::create(); });

    registry.register_node(NodeType::ParticleSystem, "Generators", "Manages a system of 2D particles with configurable behavior",
